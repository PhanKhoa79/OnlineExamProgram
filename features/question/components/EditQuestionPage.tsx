...
import { SaveOutlined, Create, Add, Delete, Info } from "@mui/icons-material";
...
export default function EditQuestionPage({ id }: { id: number }) {
  ...
  const [currentQuestionData, setCurrentQuestionData] = useState<QuestionDto | null>(null);

  // Fetch subjects and question data on component mount
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (questionData) {
          ...
          setAudioUrl(questionData.audioUrl || "");
          setPassageText(questionData.passageText || "");
          setDifficultyLevel(questionData.difficultyLevel || "trung bình");
          setSubjectId(questionData.subjectId || undefined);
          setAnswers(questionData.answers.map(answer => ({
            id: answer.id,
          ...
          toast({
            title: "Lỗi khi lấy thông tin câu hỏi",
            variant: "error",
          });
        }
      } catch (error) {
  ...
  const updateAnswer = (index: number, field: keyof UpdateAnswerDto, value: string | boolean) => {
  ...
  const handleSubmit = async (exitAfterSave = false) => {
    ...
    try {
      ...
      // Cập nhật currentQuestionData ngay lập tức
  ...
  const getSubjectName = (subjectId: number | null) => {
    ...
    return subject ? `${subject.name} (${subject.code})` : `ID: ${subjectId}`;
  };
    ...
    <div className="space-y-6 px-6 py-4">
      <NavigableBreadcrumb
        items={[
          ...
          { label: "Sửa câu hỏi", isActive: true },
        ]}
      />
      ...
      <div className="text-center space-y-3 py-6">
        ...
        <p className="text-lg text-gray-600 font-medium">
          Chỉnh sửa câu hỏi: <span className="text-blue-600 font-semibold bg-blue-50 px-3 py-1 rounded-full">&quot;{questionText}&quot;</span>
        </p>
      </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          ...
          <div className="space-y-2">
            <Label>Môn học *</Label>
            <Select value={subjectId?.toString()} onValueChange={(value) => setSubjectId(Number(value))}>
              <SelectTrigger>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 text-sm">
              <div className="space-y-1">
                <Label className="font-semibold text-gray-600">ID câu hỏi:</Label>
                <p className="text-gray-800">{currentQuestionData.id}</p>
              </div>
              <div className="space-y-1">
                <Label className="font-semibold text-gray-600">Môn học:</Label>
                <p className="text-gray-800">{getSubjectName(currentQuestionData.subjectId)}</p>
              </div>
              <div className="space-y-1">
                <Label className="font-semibold text-gray-600">Độ khó:</Label>
                <p className="text-gray-800 capitalize">{currentQuestionData.difficultyLevel || "Chưa xác định"}</p>
              </div>
              <div className="space-y-1">
                <Label className="font-semibold text-gray-600">Số câu trả lời:</Label>
                <p className="text-gray-800">{currentQuestionData.answers?.length || 0}</p>
              </div>
              <div className="space-y-1">
                <Label className="font-semibold text-gray-600">Ngày tạo:</Label>
                <p className="text-gray-800">{formatDateTime(currentQuestionData.createdAt)}</p>
              </div>
              <div className="space-y-1">
                <Label className="font-semibold text-gray-600">Cập nhật lần cuối:</Label>
                <p className="text-gray-800">{formatDateTime(currentQuestionData.updatedAt)}</p>
              </div>
            </div>
          </CardContent>
        </Card>
